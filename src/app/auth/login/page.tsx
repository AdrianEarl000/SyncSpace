...
// 1. We move the form and hooks into a separate component
function LoginForm() {
  ...
  return (
    <>
      <h1 className="font-clash font-bold text-2xl mb-1" style={{ color: "var(--text)" }}>Welcome back</h1>
      <p className="text-sm mb-7" style={{ color: "var(--muted)" }}>Sign in to your workspace</p>

      {/* Google OAuth */}
      <button
        onClick={() => { setGoogleLoading(true); signIn("google", { callbackUrl }); }}
        disabled={googleLoading}
        className="w-full flex items-center justify-center gap-3 py-3 px-4 rounded-xl text-sm font-medium transition-all hover:scale-[1.01] mb-5 disabled:opacity-60"
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div>
          ...
          <input type="email" value={email} onChange={(e) => setEmail(e.target.value)}
            required placeholder="you@company.com" className="input-base" />
        </div>
        <div>
          <label className="block text-xs font-semibold mb-1.5 uppercase tracking-wide" style={{ color: "var(--muted)" }}>Password</label>
...
// 2. The main page component wraps LoginForm in Suspense
export default function LoginPage() {
  return (
    <div className="min-h-screen flex items-center justify-center p-4 noise relative"
      ...
      <div className="relative z-10 w-full max-w-md">
        ...
        <div className="card rounded-3xl p-8" style={{ background: "var(--sidebar)" }}>
          <Suspense fallback={<div className="text-center text-[var(--muted)]">Loading form...</div>}>
            <LoginForm />
          </Suspense>
        </div>
      </div>
